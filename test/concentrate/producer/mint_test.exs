defmodule Concentrate.Producer.MintTest do
  @moduledoc false
  use ExUnit.Case, async: false
  import Concentrate.Producer.Mint
  import Plug.Conn, only: [get_req_header: 2, put_resp_header: 3, send_resp: 3]

  doctest Concentrate.Producer.Mint
  ...
  describe "init/1" do
    test "parser can be a module" do
      assert {:producer, state, _} = init({"http://url", parser: __MODULE__.TestParser})
    end

    test "parser can be a module with options" do
      assert {:producer, state, _} =
               init({"http://url", parser: {__MODULE__.TestParser, [opt: 1]}})
    end
  end
  ...
  describe "handle_info/2" do
    ...
    test "ignores unknown messages" do
      {:producer, state, _} = init({"url", parser: &[&1]})
      assert {:noreply, [], ^state} = handle_info(:unknown, state)
    end

    @tag :capture_log
    test "does not send more demand than requested" do
      {:producer, state, _} = init({"url", parser: &[&1], demand: 1})
      first_response = make_ref()

      responses = [
        {:status, first_response, 200},
        {:headers, first_response, []},
        {:data, first_response, "body"},
        {:done, first_response}
      ]

      {state, events} =
        Enum.reduce(responses, {state, []}, fn response, {state, events} ->
          case handle_info(response, state) do
      ...
      second_response = make_ref()

      responses = [
        {:status, second_response, 200},
        {:headers, second_response, []},
        {:data, second_response, "body"},
        {:done, second_response}
      ]
  ...
  describe "bypass" do
    ...
    test "schedules a fetch again if there's a disconnection", %{bypass: bypass} do
      ...
      {:ok, producer} = start_producer(bypass, fetch_after: 50, get_opts: [timeout: 100])
    ...
    test "a fetch error is not fatal" do
      {:ok, pid} =
        start_supervised({Concentrate.Producer.Mint, {"http://nodomain.dne", parser: & &1}})
    ...
    defp start_producer(bypass, opts \\ []) do
      ...
      {:ok, _} = start_supervised({Concentrate.Producer.Mint, {url, opts}})
    end
